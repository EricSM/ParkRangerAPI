...
class Report:
    def __init__(self, loc_name, loc_lat, loc_long, description, severity, closure):
        ...
        self.closure = 0 # Bit
        self.report_datetime = dt.now().strftime("%m/%d/%Y, %H:%M:%S") # String

    def set_id(self, id):
        self.id = id

    def insert_string(self):
        return "{}, {}, {}, {}, {}, {}, {}".format(self.loc_name, str(self.loc_lat), str(self.loc_long), self.description, str(self.severity, str(self.closure), self.report_datetime))
...
def main():
    print("Main")
    rh = ReportHandler()
    rh.create_report("test", 1.0, 1.0, "some description", 10, 0)
